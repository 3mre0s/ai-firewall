...
The protected child receives temporary Apps-disabled and request-compression-disabled overrides, so Codex Apps/MCP traffic does not start outside the inspected model-provider route. Forwarded arguments cannot change that route, the `anonmyz` provider definition, Apps, or request-compression features. The current-CLI probe accepts only a literal loopback model URL and fails if any request reaches its external-egress trap; it does not allowlist `chatgpt.com`, OpenAI, or any other hostname.
