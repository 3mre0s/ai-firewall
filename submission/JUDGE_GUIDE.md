...
The protected child receives temporary Apps-disabled and request-compression-disabled overrides. Forwarded arguments cannot change the protected provider route, the `anonmyz` provider definition, Apps, or request-compression features. Verification fails if Apps/MCP or any other unexpected request reaches the external-egress trap; no OpenAI or `chatgpt.com` hostname is allowlisted.
